...
static void CursorCb_ModeSleep(u8);
...
static u8 DisplaySelectionWindow(u8 windowType)
{
    ...
    for (i = 0; i < sPartyMenuInternal->numActions; i++)
    {
        ...
        if (sPartyMenuInternal->actions[i] == MENU_MODE)
            fontColorsId = 4;
...
static void SetPartyMonFieldSelectionActions(struct Pokemon *mons, u8 slotId)
{
    ...
    if (!InBattlePike() && GetMonData(&mons[1], MON_DATA_SPECIES) != SPECIES_NONE)
    ...
    else
    {
        ...
        if (sFinalLevel > sInitialLevel)
        {
            ...
            else // Exp Candies
            {
                ConvertIntToDecimalStringN(gStringVar2, sExpCandyExperienceTable[holdEffectParam - 1], STR_CONV_MODE_LEFT_ALIGN, 6);
                ConvertIntToDecimalStringN(gStringVar3, sFinalLevel, STR_CONV_MODE_LEFT_ALIGN, 3);
                if (gSpecialVar_ItemId == ITEM_HYPER_CANDY)
...
static void CursorCb_ModeSleep(u8 taskId)
{
    PartyMenuModeSetStatus(taskId, STATUS1_SLEEP);
}
