...
s32 GetDrainedBigRootHp(enum BattlerId battler, s32 hp)
{
    ...
    {
        // Big Root uses holdEffectParam as a percent bonus (30 -> 1.3x).
        hp = (hp * (100 + GetBattlerHoldEffectParam(battler))) / 100;
    }
    if (hp == 0)
        hp = 1;
...
enum Obedience GetAttackerObedienceForAction(void)
{
    ...
    if (FlagGet(FLAG_BADGE02_GET)) // Knuckle Badge
        obedienceLevel = 100;
    if (FlagGet(FLAG_BADGE03_GET)) // Balance Badge
        obedienceLevel = 100;
    if (FlagGet(FLAG_BADGE04_GET)) // Bolt Badge
        obedienceLevel = 100;
    if (FlagGet(FLAG_BADGE05_GET)) // Heat Badge
        obedienceLevel = 100;
